...
def check_suspicious_combinations(counts_g: dict, counts_m: dict, benign_ratio: float) -> Tuple[float, List[str]]:
    ...
    combinations = {
        ...
        ('admin', 'network'): {'score': 4.5, 'desc': 'Cihaz yöneticisi yetkileriyle ağ iletişimi'},
        ...
        ('location', 'camera'): {'score': 3.5, 'desc': 'Konum + Kamera: Stalkerware/Gözetleme uygulaması'},
        ('microphone', 'exfiltration'): {'score': 4.0, 'desc': 'Ses kaydı + Veri sızdırma: Dinleme trojanı'},
        ('clipboard', 'network'): {'score': 3.5, 'desc': 'Pano okuma + Network: Şifre/Token hırsızlığı'},
        ('webview', 'overlay'): {'score': 4.0, 'desc': 'WebView + Overlay: Web-based phishing attack'},
        ('notifications', 'exfiltration'): {'score': 3.5, 'desc': 'Bildirim okuma + Sızdırma: 2FA code theft'},
        ('vpn', 'exfiltration'): {'score': 4.5, 'desc': 'VPN service + Sızdırma: Traffic interception'},
        ('native', 'obfuscation'): {'score': 3.5, 'desc': 'Native kod + Gizleme: Anti-analiz tekniği'},
        ('anti_debug', 'obfuscation'): {'score': 3.0, 'desc': 'Anti-debug + Obfuscation: Analiz karşıtı'},
        ('bluetooth', 'exfiltration'): {'score': 3.0, 'desc': 'Bluetooth + Sızdırma: Yakın cihazlara veri aktarımı'},
        ('calendar', 'exfiltration'): {'score': 3.2, 'desc': 'Takvim + Sızdırma: Hassas randevu bilgisi çalma'},
        ('sensor', 'exfiltration'): {'score': 2.8, 'desc': 'Sensör + Sızdırma: Davranış profili çıkarma'},
    }
    ...
    triple_combinations = {
        ...
        ('location', 'camera', 'microphone'): {'score': 6.0,
                                               'desc': 'STALKERWARE: Konum + Kamera + Mikrofon gözetleme'},
        ('location', 'camera', 'exfiltration'): {'score': 5.5, 'desc': 'Lokasyon-based gözetleme + Veri sızdırma'},
        ('webview', 'overlay', 'keylogging'): {'score': 5.5, 'desc': 'WebView phishing + Overlay + Keylog'},
        ('clipboard', 'notifications', 'network'): {'score': 5.0, 'desc': '2FA/OTP bypass: Pano + Bildirim + Network'},
        ('vpn', 'crypto', 'network'): {'score': 5.0, 'desc': 'VPN-based encrypted traffic analysis'},
        ('native', 'anti_debug', 'obfuscation'): {'score': 5.0,
                                                  'desc': 'Advanced evasion: Native + AntiDebug + Obfuscation'},
        ('microphone', 'location', 'exfiltration'): {'score': 5.5, 'desc': 'Audio surveillance + Lokasyon tracking'},
        ('package_info', 'accessibility', 'keylogging'): {'score': 5.5,
                                                          'desc': 'Hedefli keylogging: App detection + Accessibility'},
        ('calendar', 'contacts', 'exfiltration'): {'score': 4.8, 'desc': 'Sosyal mühendislik verisi toplama'},
        ('bluetooth', 'location', 'exfiltration'): {'score': 4.5, 'desc': 'Proximity-based tracking + Veri çalma'},
        ('webview', 'file_operations', 'exfiltration'): {'score': 5.0,
                                                         'desc': 'WebView download + File access + Sızdırma'},
        ('sensor', 'location', 'network'): {'score': 4.5, 'desc': 'Davranış profilleme: Sensör + Konum + C2'},
        ('admin', 'persistence', 'network'): {'score': 5.5, 'desc': 'Rootkit-like: Admin + Kalıcılık + C2'},
        ('reflection', 'obfuscation', 'dynamic'): {'score': 5.0,
    ...
    quad_combinations = {
        ...
        ('location', 'camera', 'microphone', 'exfiltration'): {'score': 7.0,
                                                               'desc': 'FULL SURVEILLANCE: Konum + Kamera + Mikrofon + Sızdırma'},
        ('webview', 'overlay', 'keylogging', 'exfiltration'): {'score': 6.5, 'desc': 'Advanced web phishing chain'},
        ('clipboard', 'notifications', 'accessibility', 'network'): {'score': 6.5, 'desc': 'Complete 2FA bypass chain'},
        ('vpn', 'crypto', 'network', 'exfiltration'): {'score': 6.5, 'desc': 'Encrypted traffic hijacking + Sızdırma'},
        ('native', 'anti_debug', 'obfuscation', 'dynamic'): {'score': 6.0,
                                                             'desc': 'Maximum evasion: Full anti-analysis stack'},
        ('package_info', 'accessibility', 'overlay', 'keylogging'): {'score': 6.8,
                                                                     'desc': 'Targeted credential theft: App detection + Full input capture'},
        ('admin', 'persistence', 'shell_exec', 'network'): {'score': 6.5,
                                                            'desc': 'Rootkit behavior: Admin + Persistence + Shell + C2'},
        ('location', 'contacts', 'calendar', 'exfiltration'): {'score': 6.0, 'desc': 'Complete personal data theft'},
        ('bluetooth', 'location', 'sensor', 'exfiltration'): {'score': 5.5,
                                                              'desc': 'IoT/Proximity-based tracking system'},
        ('webview', 'file_operations', 'crypto', 'exfiltration'): {'score': 6.0,
    ...
    penta_combinations = {
        ...
        ('location', 'camera', 'microphone', 'contacts', 'exfiltration'): {'score': 8.0,
                                                                           'desc': 'ULTIMATE STALKERWARE: Full surveillance + Contact data'},
        ('accessibility', 'overlay', 'keylogging', 'clipboard', 'network'): {'score': 8.0,
                                                                             'desc': 'Complete input interception system'},
        ('package_info', 'accessibility', 'overlay', 'sms', 'banking'): {'score': 8.5,
                                                                         'desc': 'Advanced Banking Trojan: Full capability + Targeting'},
        ('native', 'anti_debug', 'obfuscation', 'dynamic', 'root_detection'): {'score': 7.5,
                                                                               'desc': 'Military-grade evasion: Complete anti-analysis'},
        ('admin', 'persistence', 'crypto', 'network', 'shell_exec'): {'score': 8.0,
                                                                      'desc': 'Advanced persistent threat (APT-like)'},
        ('vpn', 'crypto', 'network', 'exfiltration', 'admin'): {'score': 7.5,
                                                                'desc': 'Enterprise data theft: VPN hijacking + Admin'},
        # ... diğerleri korunur
    }

    # --- VERY HIGH THREAT (senin verdiğin blokların mantığını doğrudan uyguluyoruz) ---
    ...
    if is_very_high_threat:
        if very_high_threat_combo_1:
            ...
            detected_combinations.append("VERY_HIGH: Accessibility + Overlay + BankingTargets (+8.5)")
        if very_high_threat_combo_2:
            suspicious_score += 8.0
            detected_combinations.append("VERY_HIGH: Admin + Crypto (Ransomware/privilege chain) (+8.0)")
        if very_high_threat_combo_3:
            suspicious_score += 8.0
            detected_combinations.append("VERY_HIGH: SMS + Network + DynamicCode (OTP/Exfiltration chain) (+8.0)")
        if very_high_threat_combo_4:
            suspicious_score += 7.5
            detected_combinations.append("VERY_HIGH: DynamicCode + ShellExec (Dynamic loader + shell) (+7.5)")
        if very_high_threat_combo_5:
            suspicious_score += 8.5
            detected_combinations.append(
                "VERY_HIGH: Location + Camera + Microphone + Exfiltration (STALKERWARE) (+8.5)")
        if very_high_threat_combo_6:
            suspicious_score += 8.0
            detected_combinations.append(
                "VERY_HIGH: Clipboard + Notifications + Accessibility + C2 (2FA Bypass) (+8.0)")
        if very_high_threat_combo_7:
            suspicious_score += 8.0
            detected_combinations.append("VERY_HIGH: VPN + Crypto + Exfiltration (Traffic Hijacking) (+8.0)")
        if very_high_threat_combo_8:
            suspicious_score += 7.5
            detected_combinations.append(
                "VERY_HIGH: WebView + Overlay + Keylogging + Exfiltration (Web Phishing) (+7.5)")
        if very_high_threat_combo_9:
            suspicious_score += 8.0
            detected_combinations.append("VERY_HIGH: Crypto + Exfiltration + Admin (Data Ransom) (+8.0)")
        if very_high_threat_combo_10:
            suspicious_score += 8.5
            detected_combinations.append("VERY_HIGH: Admin + Persistence + ShellExec + C2 (Rootkit) (+8.5)")
        if very_high_threat_combo_11:
            suspicious_score += 8.0
            detected_combinations.append(
                "VERY_HIGH: PackageInfo + Accessibility + Overlay + SMS (Targeted Banking) (+8.0)")

    if is_high_threat:
        if high_threat_combo_1:
            suspicious_score += 6.5
            detected_combinations.append("HIGH: Native + AntiDebug + Obfuscation + Dynamic (Advanced Evasion) (+6.5)")
        if high_threat_combo_2:
            suspicious_score += 7.0
            detected_combinations.append("HIGH: Location + (Camera|Microphone) + Exfiltration (Surveillance) (+7.0)")
        if high_threat_combo_3:
            suspicious_score += 6.5
            detected_combinations.append("HIGH: Accessibility + Keylogging + Clipboard + C2 (Credential Theft) (+6.5)")
        if high_threat_combo_4:
            suspicious_score += 6.5
            detected_combinations.append("HIGH: Persistence + ShellExec + C2 (Backdoor) (+6.5)")
        if high_threat_combo_5:
            suspicious_score += 6.0
            detected_combinations.append("HIGH: Contacts + SMS + Exfiltration (Social Engineering) (+6.0)")
        if high_threat_combo_6:
            suspicious_score += 6.5
            detected_combinations.append("HIGH: RootDetection + ShellExec + Admin (Privilege Escalation) (+6.5)")
    # Eğer medium threat türevleri varsa, küçük ek skorlar ver
    if is_medium_threat:
        if medium_threat_combo_1:
            suspicious_score += 4.5
            detected_combinations.append("MEDIUM: DynamicCode + (Accessibility|Keylogging) (+4.5)")
        if medium_threat_combo_2:
            suspicious_score += 4.5
            detected_combinations.append("MEDIUM: Obfuscation + DynamicCode (Packing) (+4.5)")
        if medium_threat_combo_3:
            suspicious_score += 5.0
            detected_combinations.append("MEDIUM: Crypto + C2 + Obfuscation (Hidden C2) (+5.0)")
        if medium_threat_combo_4:
            suspicious_score += 4.0
            detected_combinations.append("MEDIUM: Overlay + PackageInfo (Phishing Prep) (+4.0)")
        if medium_threat_combo_5:
            suspicious_score += 4.5
            detected_combinations.append(
                "MEDIUM: Persistence + (Location|Camera|Microphone) (Background Surveillance) (+4.5)")
        if medium_threat_combo_6:
            suspicious_score += 4.0
            detected_combinations.append("MEDIUM: (Contacts+SMS) | (Clipboard+Notifications) (Data Collection) (+4.0)")
        if medium_threat_combo_7:
            suspicious_score += 4.5
            detected_combinations.append("MEDIUM: Reflection + (Obfuscation|Native) (Evasion) (+4.5)")
        if medium_threat_combo_8:
            suspicious_score += 4.0
            detected_combinations.append("MEDIUM: VPN | (C2+Crypto) (Network Manipulation) (+4.0)")
    ...
    if benign_ratio < 0.70:
        ...
        for critical in CRITICAL_COMBO_PATTERNS:
            if all(flag in active_flags for flag in critical):
                ...
                detected_combinations.append(f"{desc} (+6.0)")
                suspicious_score += 6.0
